Update 2026-07-12: `apply_chain(chain)` and registry-level chain UX/metrics are implemented and tested; next safe slice is Browser QA when available, then decide whether a separate manager chain dashboard is needed.

...
Следующая задача:
Не начинать заново `dashboard-work-queue-financial-integrity-signal`: он выполнен и проверен. Перед новым кодом выбрать или создать следующий явный контракт. Возможные безопасные направления: production-scale/cached financial audit surfacing, ledger summary/backfill planning, либо другой user-approved доменный срез. Не менять operations/models.py, migration chain, billing.apply_decision semantics, payroll/grant semantics, статусы или auto-fix/backfill без нового контракта.

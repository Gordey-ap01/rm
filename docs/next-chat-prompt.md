Update 2026-07-12: `apply_chain(chain)`, registry-level chain UX/metrics, dashboard/work queue chain signals, chain attention ordering (`failed` -> `stale` -> `ready`), work queue next-action guidance, detail-page chain diagnostics for stale/blocked/failed chains, direct work-queue-to-detail chain deep links (`#chain-<id>`), registry row priority chain deep links, chain POST anchor redirects, terminal-chain read-only actions, and step-level POST anchor redirects (`#step-<id>`) are implemented and tested. Browser QA passed for work queue guidance, chain detail diagnostics, work queue chain deep links, registry chain deep links, chain/step POST redirect anchor preservation, and terminal chain action visibility. A separate manager chain dashboard is deferred until real usage shows a concrete gap; next safe step is another non-DB operations slice or a fresh contract before DB/ledger/payroll work.

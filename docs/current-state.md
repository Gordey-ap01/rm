...
Срезы 61A фиксированной групповой серии и 61B безопасного присоединения
каскада к существующим группам реализованы, локально приняты и подтверждены
GitHub Actions run `33169757672`. Для 61C принят ADR-009: устойчивый корень,
типизированные редакции, append-only запуски/попытки и единая материализация.
Следующий кодовый подсрез - 61C-1, аддитивная модель редакций и dual-write без
удаления legacy-полей.
...
1. Реализовать 61C-1: аддитивные revision/run-модели, dual-write и безопасный
   legacy backfill; затем 61C-2 единый materializer, 61C-3 будущий состав и
   61C-4 явные режимы повтора/отмены без изменения фактов прошлого.
2. Затем закрыть 61D: паузу/отмену, переходы программы/каскада, registry,
   руководительские отчеты и полную ролевую приемку серий.
3. Перед production migration chain `0048-0056` выполнить backup v2, `--strict`
   preflight, временно закрыть грантовые записи и подготовить совместимый
   rollback-релиз. После появления истории `0053`/`0054` и occurrences `0055-0056`
...
- 61C-61D: версии будущего состава и полный жизненный цикл серий;
...
- production cutover migration chain `0048-0056` после backup v2/preflight.

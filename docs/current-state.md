...
Дата контрольной точки: 2026-09-09

61D-4 реализован и локально принят: в карточке серии доступны остановка новых
запусков администратором/руководителем и возобновление только руководителем.
Каждое действие требует основания, UUID и версии последнего решения.
Повтор принятого UUID не пишет второе событие, устаревшая форма получает 409.
Возобновление разрешено только после последней явной остановки; отмена будущих
занятий и снятие участий не считаются временной остановкой. Правило закреплено
в сервисе, модели и PostgreSQL guard `0066`; миграция проверяет старую историю
и отказывается от некорректного продолжения без автоматической перезаписи.
Карточка, история и фильтр реестра различают остановленные и отмененные серии,
включая отмененные legacy-серии без журнала решений.

Локальная приемка: новый модуль — 13 тестов на PostgreSQL 17 без пропусков;
на SQLite — 13 тестов, один PostgreSQL-only пропуск. Проверены service/ORM/raw
SQL, UUID replay после позднейших событий, stale form, роли, сохранность фактов
и forward/reverse/preflight `0066`. Еще 183 существующих теста серий, реестра
и редактора состава прошли на PostgreSQL; 10 тестов реестра — на SQLite после
исправления NULL-фильтра старых отмененных серий. Чистая PostgreSQL-цепочка
`0001-0066` применена к отдельной синтетической БД.
Browser QA подтвердил stop/resume create/join, stale POST 409, отказ resume
после снятия участия, роли, фильтры и новые подписи истории. После двух циклов
stop/resume сохранены 4 занятия, 9 участий, 8 назначений и исходная проводка.
Формы, карточка и реестр проверены в Chrome на 390px без переполнения и ошибок
консоли. Встроенный браузер не применял viewport override; мобильная приемка
выполнена в Chrome, а не выведена из desktop-скриншота. Независимое ревью
не нашло блокирующих замечаний. Общий CI этого среза еще не завершен.

Docker Desktop восстановлен после зависших локальных socket-файлов: только
runtime-каталоги переименованы с сохранением копий; базы, тома и настройки
не сбрасывались. Рабочая БД и production-развертывание не затрагивались.

Предыдущая принятая контрольная точка 61D-3:

61D-3 реализован: в карточке активной create-серии доступен редактор будущего
состава получателей и специалистов. Перед сохранением обязателен просмотр
точных данных; подписанное подтверждение связано с автором и исходной
редакцией. Устаревшая форма и повторное сохранение не создают вторую редакцию.
Уже созданные занятия, их участники, назначения и финансовые факты сохраняются;
новые занятия автоматически не создаются. История редакций показывает автора,
роль, основание, дату применения и полный состав, по 10 редакций на страницу.
Руководитель переопределяет администратора; будущую редакцию руководителя
может заменить только руководитель. Его первичная created/legacy_import
редакция не блокирует оперативную работу администратора.

Локальная приемка: 15 новых тестов редактора на SQLite и PostgreSQL 17,
196 существующих тестов серий и затронутых экранов на PostgreSQL. Browser QA
на отдельной синтетической БД подтвердил preview/apply, замену получателя,
добавление/удаление строки специалиста, необходимость нового просмотра после
правки формы, решения администратора/руководителя и запрет специалисту.
История сохраняет прежний состав; существующее будущее занятие осталось
с прежними участниками. Редактор и история на 390px без переполнения,
ошибок консоли на рабочих экранах нет. Схема и рабочая БД не менялись.
Код `f06aefc` отправлен в GitHub; общий
[CI 34236438524](https://github.com/Gordey-ap01/rm/actions/runs/34236438524)
успешен: чистые migrations/configuration/lint/manifests/Compose и production
backup/restore drill прошли. Полный pytest — `1101 passed, 486 warnings`
за `817.99s`; весь job занял 21 минуту 48 секунд, restore drill — 7 минут.
61D-3 принят локально и в общем CI. После обрыва связи продолжать со следующего
среза, не повторять эту приемку без новых изменений.
Browser-сервер остановлен, временный PostgreSQL-контейнер удален.
Независимое финальное ревью не нашло блокирующих замечаний. Проверки Django,
Ruff, migration/diff и dependency manifests прошли. Прежняя принятая точка — ниже.

Предыдущая принятая контрольная точка 61D-2:

61D-2 реализован: реестр серий открывается из расписания, карточки получателя
и каждой программы. Фильтры получателя/программы учитывают legacy-связь,
групповой состав и все сохраненные редакции; снятие участника из будущего
состава не скрывает историю. Статус и включительный плановый период
проверяются формой; неверные фильтры показывают ошибки и пустой результат.
Пагинация по 25 строк сохраняет фильтры. Управление остается в существующей
карточке retry/cancel/withdraw с прежними полномочиями.

Локальная приемка 61D-2 завершена: 10 новых тестов на PostgreSQL 17 и SQLite,
142 существующих теста серий и затронутых экранов на PostgreSQL, Django check,
Ruff, migration/diff check и manifests прошли. Проверено постоянное число
запросов при росте числа серий и редакций. Browser QA на отдельной синтетической
PostgreSQL-БД подтвердил администратора/руководителя, запрет специалисту,
историческую принадлежность, переходы из программы/расписания, пагинацию,
ошибочный период и 390px без переполнения; ошибок консоли на рабочих экранах нет.
Новых миграций и изменений рабочей БД нет. Код `a97232a` отправлен в GitHub;
[общий CI 34229677232](https://github.com/Gordey-ap01/rm/actions/runs/34229677232)
завершился успешно: migration/configuration/lint/manifests/Compose и backup/restore
drill прошли; полный pytest — `1086 passed, 471 warnings` за 13 минут 9 секунд.
Весь job занял 22 минуты 7 секунд. Временная PostgreSQL-БД удалена,
browser-сервер остановлен. 61D-2 принят локально и в CI; после обрыва связи
продолжать со следующего среза, не повторять эту приемку без новых изменений.

Предыдущая принятая контрольная точка:

61D-1: рабочие retry/cancel/withdraw реализованы; browser-приемка действий,
обязательного основания, приоритета руководителя и ограничения специалиста
завершена. На ширине 390px нет переполнения, ошибок консоли нет. Снятые
join-участия вынесены в историю административной карточки и исключены из
активного состава и полей отметки специалиста.

PRD 5.7 реализован по контракту `62-operator-attendance-and-schedule-decisions.md`:
из карточки занятия администратор и руководитель отмечают проведение и
принимают расписание без входа/ответа специалиста и без предварительного
согласования. Миграция `0065` добавляет два неизменяемых журнала с фактическим
автором, ролью, основанием, UUID операции и цепочкой исправлений. Решение
руководителя меняет только руководитель; поздний ответ специалиста не меняет
итог центра. Расписание принимается для конкретного специалиста и времени.
Ручное действие не выдается за ответ специалиста и не создает финансовых
операций. Исправление проведения после финансовых фактов требует отдельного
процесса корректировки и в этом срезе блокируется.

Локально проверены чистая PostgreSQL 17 migration chain `0001-0065`, Django
check, отсутствие незаписанных миграций, Ruff и diff check. Browser QA подтвердил
администратора, переопределение руководителем, защиту от позднего POST,
принятие расписания без писем и отсутствие лишних финансовых записей.
Ручное принятие также синхронизирует уже существующие обычные запросы данного
специалиста: они не остаются `pending`, остальные адресаты и шаги переноса
не изменяются. Этот сценарий прошел browser smoke; модуль согласований на
PostgreSQL — `18 passed`, включая legacy write-path и replay нового UUID.
Полный модуль серий прошел на PostgreSQL: `158 passed`. Все 26 сценариев
двух новых модулей проверены на PostgreSQL, включая прямой INSERT с ложной
ролью/чужим назначением, неизменяемость, гонку решений и reverse `0065`;
после исправления одного тестового assertion его повторный запуск прошел.
На отдельной чистой БД проверено реальное подключение ограниченной runtime-role:
принятие расписания, проведение и переопределение руководителем из группы
успешны, прямой UPDATE обоих журналов запрещен правами PostgreSQL.
Общая приемка завершена: код `299d0b2`, [GitHub Actions run 34141717376](https://github.com/Gordey-ap01/rm/actions/runs/34141717376)
успешен. Чистые PostgreSQL migrations, configuration/migration check, Ruff,
manifests, Compose и production backup/restore drill прошли; полный pytest —
`1076 passed, 461 warnings`, без failures, 13 минут 39 секунд. Весь job занял
22 минуты 10 секунд. Прежний лимит 20 минут был недостаточен; он увеличен до
30 минут без сокращения проверок. Ранее обнаруженные четыре устаревшие проверки
ролей/текста приведены к текущему контракту. Docker и GitHub доступны.
Browser-серверы остановлены, временные PostgreSQL-контейнеры удалены.
Production-развертывание и миграция рабочей БД в этом срезе не выполнялись.
...
чужой состав и ledger сохраняются. Рабочие команды 61D-1 и полномочия по
PRD 5.7 реализованы; итог общей приемки указан выше. Далее — остальной 61D UI.
...
- `docs/62-operator-attendance-and-schedule-decisions.md`.
...
1. Закрыть общий CI 61D-4 и записать принятую контрольную точку. Затем —
   оставшийся 61D: переходы программы/каскада, руководительские отчеты и общая
   ролевая приемка. Ближайший срез ограничить конкретными переходами и заранее
   определить последствия для серий, существующих занятий и финансов.
   До push проверять новые сценарии и существующие классы измененных экранов;
   перед PostgreSQL-прогоном явно подтверждать vendor и port соединения.
2. Перед production migration chain `0048-0066` выполнить backup v2, `--strict`
   ...
   и operator history `0065` эти миграции не откатываются:
   rollback сохраняет additive schema, private volume и immutable-историю.
   `0066` выполняет preflight старых resume-событий; его reverse ослабляет guard
   и не является production-стратегией отката приложения.
3. Закрыть production-допуск 59B-2: transactional notification outbox перед
...
4. Отдельно согласовать policy возвратов/обратной конвертации и исправления
   проведения после финансовых фактов; перед пилотом проверить
...
- 61D: оставшийся UI полного жизненного цикла серий и общая приемка;
...
- production cutover migration chain `0048-0066` после backup v2/preflight.
...
- Не откатывать `0065` после первого решения о проведении или расписании:
  rollback сохраняет журналы и использует совместимую версию приложения.

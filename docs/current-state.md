...
Общий CI для нового кода еще ожидается; прежняя принятая точка — ниже.
...
1. Завершить общий CI для редактора будущего состава 61D-3 и записать принятый
   commit/run. Затем — безопасный UI stop/resume с обязательным hardening

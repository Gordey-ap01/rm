...
1. Следующий законченный сценарий 61D — реестр связанных серий из программы
   получателя и расписания с переходом в существующую карточку retry/cancel/withdraw.
   Новая доменная модель для этого не нужна. Параллельные зоны: registry view/template,
   ссылки из программы/расписания, независимые role/filter/navigation tests.
   До push тестовый агент проверяет также существующие классы измененных экранов;
   перед PostgreSQL-прогоном явно подтверждает vendor и port соединения.
   Затем отдельно — редактор будущего состава, безопасный UI stop/resume,
   переходы программы/каскада, руководительские отчеты и общая ролевая приемка.

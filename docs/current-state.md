...
61C-1 и 61C-2 реализованы локально: устойчивый корень, immutable-редакции,
append-only run/result, сверяемый legacy backfill, PostgreSQL guards и единый
materializer индивидуальных/групповых/create/join серий. Следующий кодовый
подсрез - 61C-3, редакция только будущего состава; затем 61C-4 retry/cancel и
61D UI/жизненный цикл.
...
1. Реализовать 61C-3: будущий состав под `expected_revision_id`, затем 61C-4
   явные режимы `missing_only`/retry/cancel без изменения фактов прошлого.
...
- 61C-3/61C-4 и 61D: будущий состав, retry/cancel и полный жизненный цикл серий;

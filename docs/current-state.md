...
грантового отчета завершены. Активный доменный этап — безопасный жизненный
цикл грантового плана по `docs/58-grant-plan-versioning-contract.md`;
параллельно нужны решения по внешней эксплуатации.
...
- Graphify code index обновлен локально без LLM-вызова: `5953` nodes,
  `26024` edges. Семантическое обновление документов не применено: внешний
...
2. Реализовать по контракту 58 append-only редакции грантового плана и
   распределений одним DB owner. Ledger-backed выделение получателю,
   фиксированная оплата проекта и снимок закрытого донорского отчета остаются
   следующими отдельными срезами.
3. Отдельно согласовать policy возвратов и обратной конвертации до реализации.
4. Перед пилотом проверить mobile-кабинет на физическом телефоне и провести
...
- append-only жизненный цикл грантового плана и закрытого донорского отчета;
...
- доработка грантовых квот по следующему опасному контракту БД.

...
1. После принятого 61D-3 — безопасный UI stop/resume с обязательным hardening
   из 61D-1, переходы программы/каскада,
   руководительские отчеты и общая ролевая приемка. До push проверять новые
   сценарии и существующие классы измененных экранов; перед PostgreSQL-прогоном
   явно подтверждать vendor и port соединения.
   Конкретный следующий hardening: `resume_materialization` должен принимать
   только непосредственно предшествующий `stop_materialization`, а не
   `cancel_future_unstarted` или `withdraw_future_joined_participations`.
   Одинаковое правило нужно закрепить в сервисе, `LifecycleEvent.clean()`
   и новой PostgreSQL guard-миграции, сохранив replay по UUID. Приемка должна
   включать отказ service/ORM/raw SQL и сохранность занятий/участий/ledger.
2. Перед production migration chain `0048-0065` выполнить backup v2, `--strict`
...
- production cutover migration chain `0048-0065` после backup v2/preflight.

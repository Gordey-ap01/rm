...
Итог нового полного CI еще ожидается. Предыдущий run `34122651901` дважды
остановился по общему timeout 20 минут на 96% pytest; лимит увеличен до 30
минут без сокращения проверок. Docker и соединение с GitHub восстановлены.
...
1. Дождаться успешного полного CI текущего среза; локальные результаты не
   подменяют общий PostgreSQL regression и production restore drill.
2. Следующий законченный сценарий 61D — реестр связанных серий из программы
...
3. Перед production migration chain `0048-0065` выполнить backup v2, `--strict`
...
5. Отдельно согласовать policy возвратов/обратной конвертации и исправления

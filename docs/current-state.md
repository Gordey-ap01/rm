...
среза полномочий и подэтап A по вместимости кабинетов завершены.
...
- Полный SQLite regression: `696 passed, 4 skipped`; пропущены только четыре
...
- PostgreSQL 17: migrations `0001-0045` и актуальный focused suite `33 passed`
  в disposable-контейнере; ручные решения и две гонки вместимости проходят.
...
1. Реализовать единый идемпотентный write-path решения о списании и его
   PostgreSQL concurrency-проверку.

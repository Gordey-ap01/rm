...
- Продолжить следующий доменный вертикальный срез только после сверки с `docs/10-reschedule-chain-dependencies-contract.md`: схема chain/dependency и построение цепочки без применения реализованы; `revalidate_chain` и `atomic apply chain` еще не реализованы.

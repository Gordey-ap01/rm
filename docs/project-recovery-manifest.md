...
- Graphify-граф найден. Полный semantic update `graphify . --update --no-viz` не проходит без LLM API key для измененных docs. Code-only update после read-only схемы цепочек ранее обновил `graphify-out/graph.json` до 3867 nodes / 13807 edges. После среза построения цепочки команда `graphify update . --no-cluster` переизвлекла 134/134 code-файла, но отказалась перезаписывать основной `graph.json`, потому что новый граф меньше существующего (`3844` vs `3867` nodes). `--force` не использовать без отдельного решения: свежие docs/code важнее graphify-query при расхождениях.
